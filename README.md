...
| シナリオ | Max RSS (kB) | 備考 |
|----------|---------------:|------|
| FFI `bun run test/ffi/test.ts` | **43672** | Linux x86_64 VPS 実測（`zig build lib` 済み・Bun・`scripts/mem-peak.sh`）。8 件の FFI 結合テスト 1 プロセスのピーク（Bun + koffi + libpict を含む）。同一条件でも数千 kB 程度ブレ得 |
| `npx tsx bench/bench.ts` | — | **成功した bench 完了後**の値を転記。要 `npm run build`（`js/dist/index.js`）+ `npm install sharp`。`js/dist` 無しで走ると途中失敗の RSS になり無効 |
