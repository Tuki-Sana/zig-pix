...
### ベンチ用フィクスチャ（`BENCH_FIXTURE`）

`bench/bench.ts` と `bench/bench-quality.ts` は環境変数 **`BENCH_FIXTURE`** で入力 PNG を切り替えられる（既定: `default`）。

| ID | ファイル | 目安 |
|----|----------|------|
| `default` | `test/fixtures/bench_input.png` | 従来の 512×512 |
| `character_kanata` | `test/fixtures/bench_fixture_character_kanata.png` | キャラ（線・グラデ） |
| `character_chika` | `test/fixtures/bench_fixture_character_chika.png` | キャラ（高彩度・細部） |
| `landscape_sunbeach` | `test/fixtures/bench_fixture_landscape_sunbeach.png` | 風景・明るい砂浜 |
| `landscape_sea` | `test/fixtures/bench_fixture_landscape_sea.png` | 風景・夕暮れ海岸 |
| `landscape_study` | `test/fixtures/bench_fixture_landscape_study.png` | 風景・建物＋岩（習作） |

`landscape_*` は作者の **一次創作**（背景・風景）、`character_*` は **二次創作**（既存作品のキャラクター等を作者が描いたイラスト）です。いずれもベンチ・再現性のため MIT の本リポジトリに収録しています。本プロジェクトは収益化を目的としません。

上記 5 枚は **長辺 1024px**（`sips -Z 1024`）に整えた作者作 PNG。別名: `character` → `character_chika`、`landscape_wide` → `landscape_sunbeach`、`landscape_soft` → `landscape_sea`。例: `BENCH_FIXTURE=character_chika npm run bench`。GitHub Actions のベンチは未指定のため **`default`** のまま。

### Node / `bench/bench.ts`（FHD・WQHD・4K 相当）

`npm run build` のうえ `npx tsx bench/bench.ts` または `npm run bench`（`sharp` は `devDependencies` に含む）。**同一パイプライン**: PNG **デコード** → **リサイズ**（出力ピクセルは表のとおり）→ **AVIF エンコード（quality=60, speed=6）**。**`BENCH_FIXTURE`** で選んだ PNG を、**計測ループの外で** Sharp（`fit=cover`）により各代表解像度へ一度だけ拡大したバイト列。各シナリオで zigpix / Sharp を **warm-up 2・計測 10**し、**wall-clock の中央値（ms）**を比較。**ratio = Sharp 中央値 ÷ zigpix 中央値**（**1 超**なら zigpix の中央値が速い）。
...
実測環境の例: **macOS arm64（Apple M）・ローカル**（2026-04-14 頃、`zigpix` 0.1.2 npm バイナリ、Sharp 0.34）。**GitHub Actions**（`ubuntu-24.04`）でも同スクリプトを実行し、成果物 `benchmark-linux-x64` に `bench/results/benchmark.md` と `benchmark.json` が添付される。
...
- **主用途はイラスト**のため、`BENCH_FIXTURE` に **キャラ**と**風景**を用意済み（上表）。トレンド解像度マトリクスは引き続き **Sharp cover での代表解像度**を優先。

品質・サイズ揃えのベンチは **`bench/bench-quality.ts`** でスパイク済み（`npm run build` のうえ **`npm run bench:quality`**。こちらも **`BENCH_FIXTURE` 可**）。Sharp をアンカーに **AVIF encode のみ**を計測し、zigpix の `quality` を走査して出力バイトを既定 ±10% 帯へ寄せる。成果物は `bench/results/benchmark-quality.json` / `benchmark-quality.md`。**CI には未接続**（手元・任意の週次ジョブ向け）。拡張するときは下のチェックリストで前提を固める。

**SSIM（簡易）**: 両出力を **Sharp で AVIF デコード**し、元ピクセル解像度に **fill** で合わせた RGB 上で **輝度のグローバル SSIM** を算出（`benchmark-quality.json` の `ssim`）。知覚的な厳密一致ではなく回帰用。下限を付ける場合は **`BENCH_QUALITY_MIN_SSIM=0.95`** のように指定（未満または計算失敗で **exit 1**）。
