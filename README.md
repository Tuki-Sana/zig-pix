...
| シナリオ | Max RSS (kB) | 備考 |
|----------|---------------:|------|
| FFI `bun run test/ffi/test.ts` | *上記スクリプトの出力から転記* | デコード・リサイズ・WebP/AVIF を含む結合テスト |
| `npx tsx bench/bench.ts` | *同上（sharp が入っている場合のみ実行）* | zigpix + sharp のベンチ |

代表値は **Linux x86_64（本番 VPS 相当）** で取ることを推奨。取得したらこの表を更新し、`docs/CHECKLIST.md` のメモリ項目を完了にできる。
