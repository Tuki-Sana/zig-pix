...
- **セマバ**は `wasm/package.json` の `version` のみ。ルートの **`zigpix` と揃える必要はない**（運用の詳細は `docs/operations.md` §9）。
...
- **npm レジストリ**: [zigpix-wasm](https://www.npmjs.com/package/zigpix-wasm)（`wasm/package.json` の `version` と一致。例: **0.1.3**）。
