...
- [x] tsukasa-art: `POST /api/admin/upload`（multipart）経路で **ブラウザ→image/webp**（`imageResize.ts` プリリサイズ）→`convertToAvif`（`zigpix@0.1.3` の静止画 WebP decode）が成立（旧メモの `/api/convert` は本リポでは未使用のため整理）
...
- **完了後**: `zigpix@0.1.3` を `tsukasa-art` に取り込み → `imageResize.ts` を WebP 送りに戻せる（転送量削減）

### 12-B: WebP encode（zigpix）✅

- **難易度**: 中
- **内容**: `libwebp` の encode API を Zig でラップし、`encodeWebp` 等を公開
- **状態**: `pict_encode_webp` / `encodeWebp()`（`js/src/index.ts`）/ E2E `decode→encodeWebP→decode` は **既に実装済み**（npm `zigpix@0.1.3`）
