...
- [x] `export fn pict_decode` — magic bytes でフォーマット判定し JPEG/PNG をデコード、RGBA/RGB ピクセルを返す (`root.zig`)
- [x] `export fn pict_resize` — Lanczos-3 リサイズ、n_threads 対応、ゼロ次元ガード付き
- [x] `export fn pict_encode_webp` — quality / lossless オプション対応 WebP エンコード
- [x] `export fn pict_free_buffer` — c_allocator バッファの解放エントリポイント
- [x] FFI ユニットテスト 4 件追加 (resize 正常系、resize ゼロ次元 null、encode_webp ヘッダ確認、decode 不正データ null)
- [x] `zig build test` / `zig build test -Dsimd=true` 全通過 (51 tests)
- [ ] Bun/Node.js FFI バインディング動作確認 (統合先ホームページプロジェクトで実施予定)
...
| 関数 | 確保側 | 解放方法 |
|------|--------|----------|
| `pict_decode` | Zig (`c_allocator`) | `pict_free_buffer(ptr, w*h*ch)` |
