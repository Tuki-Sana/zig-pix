...
- [x] Sharp との比較ベンチマーク — `bench/bench.ts`（zigpix vs sharp, decode+resize+AVIF, 中央値記録、`BENCH_FIXTURE` で作者フィクスチャ切替）+ `bench/bench-quality.ts`（encode のみ・サイズ揃え）+ `.github/workflows/bench.yml`（独立ジョブ、`continue-on-error: true`、artifact に `benchmark.json` / `benchmark.md` を 90 日保存）
...
`tsukasa-art` の `imageResize.ts` は、リサイズ後の PNG を **image/png** で送る（WebP は送らない）。理由: **zigpix の `decode()` は WebP 未対応**のため、Canvas 出力を WebP にすると VPS で `decode failed` となる。
...
### 12-A: WebP decode（zigpix）

- **難易度**: 中
- **内容**: `libwebp` を `build.zig` に追加し、`decode()` で RIFF/WEBP ヘッダを検出して分岐
- **完了後**: `zigpix` パッチリリース → `tsukasa-art` の `imageResize.ts` で PNG 入力を WebP 出力に戻せる（転送量削減）
