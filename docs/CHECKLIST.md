...
- [ ] `src/pipeline/crop.zig` を新規作成
  ...
  - **整数安全性（境界チェックより前に実施）**:
    - `left + crop_w`、`top + crop_h` を checked add（オーバーフロー → `OutOfBounds`）
    - `crop_w * crop_h * channels` を `mul3SizeChecked` 相当で検証（オーバーフロー → `OutOfMemory`）
    - 行オフセット `(top + i) * src_w * channels` も checked mul
  - 境界チェック: checked add 結果が `src_w` / `src_h` 以下であること
  - 行単位コピー（SIMD 不要、memcpy で十分）
- [ ] `src/root.zig`: `pict_crop` を export
  - シグネチャ: `pict_crop(pixels, src_w, src_h, channels, left, top, crop_w, crop_h, out_len) ?[*]u8`
  - null / ゼロ次元 / 範囲外 → null 返却、`out_len` 変更なし

#### テスト

- [ ] `src/pipeline/crop.zig`: Zig ユニットテスト
  - 正常系: 4×4 RGBA から 2×2 を切り出し、ピクセル値が正しいこと
  - 境界値: left=0, top=0（左上全体）/ 右下端
  - エラー系: 範囲外 → `OutOfBounds`、crop_w=0 → `ZeroDimension`
- [ ] `test/ffi/test.ts` など: `pict_crop` Case I（切り出し後の幅高さ・バイト長検証）

#### JS API

- [ ] `js/src/index.ts`: `crop(image, options)` 実装
  - koffi シンボル登録
  - `CropOptions` バリデーション（負値・非整数・NaN・Infinity・u32 超えをすべて JS 層で弾く）
  - ICC プロファイルを引き継ぐ
- [ ] `index.deno.ts` にも `crop` を追加
- [ ] TypeScript 型: `CropOptions` / `crop` をエクスポート

#### E2E / ドキュメント

- [ ] `test/e2e/`: `decode → crop → encodePng` でピクセル数確認
- [ ] `README.md`: `crop()` の使用例を追記
...
- [ ] `src/c/jpeg_decode.c`: `pict_jpeg_orientation(data, len) uint8_t` を追加
...
- [ ] `src/pipeline/rotate.zig`: Zig ユニットテスト
  ...
  - channels=3/4 両方
- [ ] `src/c/jpeg_decode.c` テスト: `pict_jpeg_orientation`
  - orientation 付き JPEG フィクスチャ（専用 JPEG を `test/fixtures/` に新規追加）で既知の値が返ること
  - 非 JPEG バイト列 → 1 を返す
  - 壊れた EXIF（マーカー長が範囲外）→ 1 を返す（クラッシュしない）
- [ ] `test/ffi/test.ts` など: Case J（orientation=6 な JPEG を decode → 幅高さ交換確認）

#### JS 組み込み

- [ ] `js/src/index.ts`: `decode()` 内に自動回転を組み込む
...
- [ ] テストフィクスチャ: orientation=1 / 6 / 8 の JPEG を `test/fixtures/` に追加（最低限 1 と 6、余裕があれば 8 も）
- [ ] `test/e2e/`: EXIF 付き JPEG を decode → 幅高さが正しいことを確認
- [ ] `README.md`: 自動回転の説明を追記（「JPEG の EXIF orientation は自動適用される」）

---

### Phase 13 完了条件

- [ ] `zig build test` 全通過（SIMD off/on 両方）
- [ ] `bash test/ffi/run.sh` 全 PASS（Mac）
- [ ] `npx tsx test/ffi/test.node.ts` 全 PASS
- [ ] `deno run --allow-read --allow-ffi test/ffi/test.deno.ts` 全 PASS
- [ ] E2E テスト全通過
