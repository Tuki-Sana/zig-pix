...
- [x] E2E テスト全通過
- [x] CI（build-native）グリーン
- [x] バージョン番号を `0.4.0` に更新（`package.json` / `npm/*/package.json` / `CHANGELOG.md`）
- [x] npm publish（`docs/release.md` 手順に従う）

---

## Phase 14: AVIF エンコードのマルチスレッド対応

ブランチ: `feat/avif-multithread`  
設計書: `docs/feat-avif-multithread.md`

### 概要

`encodeAvif()` に `threads` オプションを追加し、libaom の行ベース並列（`maxThreads`）を有効化する。デフォルトは `1`（シングルスレッド、現状維持）。品質への影響なし。

### 変更対象

| レイヤー | 変更内容 |
|---|---|
| `src/c/avif_encode.c` | `pict_encode_avif` に `threads` 引数追加、`encoder->maxThreads` に渡す |
| `src/root.zig` | `export fn pict_encode_avif` のシグネチャに `threads: c_int` 追加 |
| `js/src/index.ts` | `AvifOptions.threads?: number` 追加、koffi 呼び出しに渡す |
| `js/src/index.deno.ts` | 同上（Deno 側） |
| `test/ffi/test.ts` | threads=4 でのエンコード検証ケース追加 |
| `test/ffi/test.node.ts` | 同上 |
| `test/e2e/e2e.node.ts` | threads=4 で E2E 検証 |
| `test/e2e/e2e.deno.ts` | 同上 |
...
- [x] E2E テスト全通過
- [x] CI（build-native）グリーン
- [x] バージョン番号を `0.5.0` に更新（`package.json` / `npm/*/package.json` / `CHANGELOG.md`）
- [x] npm publish（`docs/release.md` 手順に従う）

---

## Phase 15 — AVIF decode ✅ (0.6.0)

`decode()` で AVIF を受け付けるようにする。libavif / libaom は既に静的リンク済みのため、デコード側の C ラッパーを追加するだけで完結。

- [x] `src/c/avif_decode.c` 実装（`pict_avif_decode` / `pict_avif_decode_free`）
- [x] `src/root.zig` にエクスポート追加
- [x] `js/src/index.ts` / `index.deno.ts` の `decode()` に AVIF 分岐追加
- [x] FFI テスト・E2E テスト追加
- [x] CI グリーン・0.6.0 npm publish 済み

---

## Phase 16 — GIF decode（先頭フレーム）✅ (0.6.0)

stb_image.h（MIT/Public Domain）を vendor に追加し、GIF の先頭フレームを RGB でデコード。アニメ GIF の 2 フレーム目以降は無視。

- [x] `vendor/stb/stb_image.h` 追加
- [x] `src/c/gif_decode.c` 実装（`pict_gif_decode` / `pict_gif_decode_free`）
- [x] `src/root.zig` にエクスポート追加
- [x] `js/src/index.ts` / `index.deno.ts` の `decode()` に GIF 分岐追加
- [x] FFI テスト・E2E テスト追加
- [x] `THIRD_PARTY_LICENSES` に stb_image.h を追記
- [x] CI グリーン・0.6.0 npm publish 済み

---

## Phase 17 — resize() fit モード ✅ (0.6.0)

`resize()` に `fit?: "stretch" | "contain" | "cover"` オプションを追加。Zig 側は `pict_resize_v2` として実装し、`pict_resize` は後方互換のため存続。

- [x] `pict_resize_v2` 実装（`fit: uint8`、`out_actual_w` / `out_actual_h` 出力）
- [x] `js/src/index.ts` / `index.deno.ts` の `resize()` に fit 分岐追加
- [x] FFI テスト・E2E テスト追加
- [x] CI グリーン・0.6.0 npm publish 済み

---

## Phase 18 — convert() パイプライン ✅ (0.6.0)

decode → crop → resize → encode を一発実行する `convert()` 関数を追加。

- [x] `js/src/index.ts` / `index.deno.ts` に `convert()` 実装
- [x] FFI テスト・E2E テスト追加
- [x] CI グリーン・0.6.0 npm publish 済み
