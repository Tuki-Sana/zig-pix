...
| 名前 | 意味 |
|------|------|
| **build-native** | GitHub Actions「Build native binaries」（`build-native.yml`）。成果物: `libpict-darwin-arm64` / `libpict-linux-x64` / **`libpict-win32-x64`** |
...
## Phase 1 — ネイティブ `zigpix`（optional 3 つ → メタパッケージ）

**0.2.0 時点**: `zigpix-darwin-arm64` / `zigpix-linux-x64` / **`zigpix-win32-x64`** の **3 optional** を先に publish してからルート `zigpix`。**`zigpix-win32-arm64`** は npm パッケージ未整備のため **同梱しない**（後続リリースで追加する場合は `docs/windows-rollout-plan.md` §4 M3 に従う）。

### 1.1 `libpict` を CI 成果物で `npm/zigpix-*/` に置く

**同じ** build-native の **1 つの RUN_ID** から、**darwin / linux / win32-x64** の artifact を取る（ジョブは並列だが run は共通）。
