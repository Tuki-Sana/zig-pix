...
| 名前 | 意味 |
|------|------|
| **build-native** | GitHub Actions「Build native binaries」（`build-native.yml`）。成果物: `libpict-darwin-arm64` / `libpict-linux-x64` / **`libpict-win32-x64`** / **`libpict-win32-arm64`**（`windows-11-arm` ジョブ） |
...
- [ ] ルート `package.json` の `version` と `optionalDependencies`、および `npm/zigpix-*/package.json` の `version` が **意図どおり**（通常は **すべて同じ**番号。**WoA 初回**など `zigpix-win32-arm64` だけ別番号の場合は §1.1a）
...
**通常**: `zigpix-darwin-arm64` / `zigpix-linux-x64` / **`zigpix-win32-x64`**（＋ WoA なら **`zigpix-win32-arm64`**）を **ルートより先に** publish してからルート `zigpix`。詳細は **`docs/windows-rollout-plan.md` §4 M3 / M5**。

### 1.1a `zigpix-win32-arm64`（Windows on ARM、初回 **0.2.1** など）

1. 緑の build-native run から **`libpict-win32-arm64`** を `gh run download`（手順は §1.1 と同型）。  
2. `libpict.dll` と LICENSE 類を **`npm/zigpix-win32-arm64/`** に置き、`npm/zigpix-win32-arm64/package.json` の **`version`** を確認。  
3. **`cd npm/zigpix-win32-arm64 && npm publish --access public`**（**ルートより前**に実行。ルート `optionalDependencies` の **`zigpix-win32-arm64` の範囲**と一致させる）。  
4. ルートを **同じメジャー・マイナー・パッチ**に上げるリリースでは、他 3 optional の **`package.json` の version** とルートの **`version`** も **揃える**ことを推奨。

### 1.1 `libpict` を CI 成果物で `npm/zigpix-*/` に置く

**同じ** build-native の **1 つの RUN_ID** から、**darwin / linux / win32-x64**（＋必要なら **win32-arm64**）の artifact を取る（ジョブは並列だが run は共通）。
...
5. **再確認**:
   ...
   ```bash
   ls -la npm/zigpix-darwin-arm64/libpict.dylib npm/zigpix-linux-x64/libpict.so npm/zigpix-win32-x64/libpict.dll
   ```

**GitHub の UI だけ使う場合**: 該当 run の **Artifacts** から `libpict-darwin-arm64` / `libpict-linux-x64` / **`libpict-win32-x64`** の zip を落とし、展開して同様に `npm/zigpix-*/` へ **dylib / so / dll** と **LICENSE / THIRD_PARTY_LICENSES** を置く。
...
# WoA 同梱リリースのみ:
# cd npm/zigpix-win32-arm64 && npm publish --access public && cd ../..
...
npm view zigpix-win32-arm64 version
```
