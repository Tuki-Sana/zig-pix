/**
 * bench/bench-threads.ts — zenpix (multi-thread AVIF) vs sharp benchmark
 *
 * Same pipeline as bench.ts: decode(PNG) → resize(outW×outH) → encodeAVIF(quality, speed)
 * The only difference: zenpix encodeAvif is called with threads=AVIF_THREADS (default: os.cpus().length).
 *
 * Env:
 *   AVIF_THREADS       zenpix encode thread count (default: os.cpus().length)
 *   BENCH_FIXTURES     comma-separated fixture ids (default: all)
 *   BENCH_WARMUP_N     warm-up iterations (default: 2)
 *   BENCH_MEASURE_N    measure iterations (default: 10)
 *   BENCH_WRITE_SAMPLES=0  skip AVIF + HTML output (JSON/Markdown only)
 *
 * Output:
 *   bench/results/benchmark-threads.json
 *   bench/results/benchmark-threads.md
 *   bench/results/report-threads.html
 *   bench/results/samples/threads-{fixture}-{scenario}-zenpix.avif
 *   bench/results/samples/threads-{fixture}-{scenario}-sharp.avif
 *
 * Run:
 *   npm run build
 *   npx tsx bench/bench-threads.ts
 *   AVIF_THREADS=4 npx tsx bench/bench-threads.ts
 */
...
async function makeInputPng(fixturePath: string, s: Scenario): Promise<Buffer> {
  ...
  return sharp(base).resize(s.inW, s.inH, { fit: "cover", position: "centre" }).png().toBuffer();
}
...
function benchZenpix(input: Buffer, outW: number, outH: number): number[] {
  ...
  for (let i = 0; i < WARMUP_N + MEASURE_N; i++) {
    ...
    const avif = encodeAvif(small, { quality: AVIF_QUALITY, speed: AVIF_SPEED, threads: AVIF_THREADS });
...
async function writeMatrixSamples(
  ...
  const small = resize(img, { width: outW, height: outH });
  const z = encodeAvif(small, { quality: AVIF_QUALITY, speed: AVIF_SPEED, threads: AVIF_THREADS });
  if (z === null) throw new Error("zenpix encodeAvif returned null (samples)");
  const zigName = `threads-${fixtureId}-${scenarioId}-zenpix.avif`;
  const sharpName = `threads-${fixtureId}-${scenarioId}-sharp.avif`;
...
console.log(
  `Benchmark (threads): decode + resize + AVIF (quality=${AVIF_QUALITY}, speed=${AVIF_SPEED}, threads=${AVIF_THREADS})`,
);
...
for (const fx of fixtures) {
  ...
  for (const s of SCENARIOS) {
    ...
    console.log(`  zenpix (threads=${AVIF_THREADS})...`);
...
const jsonResult = {
  ...
  avif: { quality: AVIF_QUALITY, speed: AVIF_SPEED, threads: AVIF_THREADS },
  iterations: { warmup: WARMUP_N, measure: MEASURE_N },
  write_samples: WRITE_SAMPLES,
  report_html: WRITE_SAMPLES ? "report-threads.html (relative to bench/results/)" : null,
...
const mdResult = `# Benchmark Results — Multithreaded AVIF (threads=${AVIF_THREADS})

**Date**: ${now}
**Runner**: ${runner}
**Pipeline**: decode PNG → resize → AVIF (quality=${AVIF_QUALITY}, speed=${AVIF_SPEED}, **threads=${AVIF_THREADS}**)
...
zenpix は \`threads=${AVIF_THREADS}\` で計測。シングルスレッド比較は \`bench/bench.ts\` 参照。

## Sample AVIFs & HTML

${
  WRITE_SAMPLES
    ? `Relative to \`bench/results/\`: \`report-threads.html\`, and \`samples/threads-{fixture}-{scenario}-zenpix.avif\` / \`…-sharp.avif\`.`
    : `Skipped (\`BENCH_WRITE_SAMPLES=0\`).`
}

## JSON

See \`benchmark-threads.json\` in this directory for machine-readable rows.
`;

const jsonPath = join(OUT_DIR, "benchmark-threads.json");
const mdPath = join(OUT_DIR, "benchmark-threads.md");
const htmlPath = join(OUT_DIR, "report-threads.html");
...
if (WRITE_SAMPLES) {
  const matrixRows: MatrixScenarioRow[] = fixtureResults.flatMap((fr) =>
    fr.scenarios.map((r) => {
      const sam = r.samples;
      if (!sam) throw new Error(`WRITE_SAMPLES set but missing sample paths for ${fr.fixture.id} / ${r.scenario.id}`);
      return {
  ...
  writeMatrixHtml(htmlPath, {
    title: `zenpix (threads=${AVIF_THREADS}) vs Sharp — bench matrix`,
    date: now,
    runner,
    pipeline: `decode PNG → resize → AVIF (threads=${AVIF_THREADS})`,
