...
- **CLI 追加**: `npx zenpix` で AVIF / WebP / PNG 変換をコマンドラインから実行できるようになった。stdin/stdout（`-`）、バッチ処理（`--out-dir`）、リサイズ（`--max-size` / `--width` / `--height` / `--fit`）、マルチスレッド AVIF（`--threads`）に対応。Python などから `subprocess` 経由で呼ぶ用途にも対応。
...
- **`encodeAvif()` に `threads` オプション追加**: libaom の行ベース並列（`encoder->maxThreads`）を有効化。デフォルト `threads: 1`（既存挙動を維持）。バッチ処理やローカル環境では `{ threads: os.cpus().length }` で高速化可能。タイル分割は使わないため**圧縮品質への影響はゼロ**。

### 破壊的変更

- **ABI 変更**: `pict_encode_avif` の C シグネチャに `uint8 threads` 引数を追加。0.4.0 以前のバイナリとは非互換（`npm install zenpix@0.5.0` で自動的に新バイナリが入る）。
